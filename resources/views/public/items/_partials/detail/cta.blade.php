...
@php
    ...
    $whatsappMessage = __('common.ui.whatsapp_message', [
    ...
    $telegramMessage = __('common.ui.telegram_message', [
        'title' => $viewModel->title,
        'id' => $viewModel->itemId
    ]);
    // Use username for Telegram link
    $telegramUrl = 'https://t.me/' . $telegramUsername . '?text=' . urlencode($telegramMessage);
@endphp

<div class="khezana-item-cta">
    @auth
        @if (!$isOwner)
            {{-- WhatsApp Button - For authenticated users only --}}
            <a href="{{ $whatsappUrl }}" 
               target="_blank" 
            ...
            {{-- Telegram Button - For authenticated users only --}}
            <a href="{{ $telegramUrl }}" 
               target="_blank" 
               rel="noopener noreferrer"
