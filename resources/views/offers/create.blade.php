        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="mb-4">
                        <h3 class="text-lg font-bold">{{ __('requests.fields.title') }}: {{ $request->title }}</h3>
                        <p class="text-sm text-gray-600">{{ $request->description }}</p>
                    </div>

                    <form method="POST" action="{{ route('offers.store', $request) }}">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="message" name="message" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" rows="4">{{ old('message') }}</textarea>
