                    ...
                    <form method="POST" action="{{ route('items.update', $item) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="operation_type" id="operation_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                                <option value="sell" {{ $item->operation_type->value == 'sell' ? 'selected' : '' }}>{{ __('items.operation_types.sell') }}</option>
                                <option value="rent" {{ $item->operation_type->value == 'rent' ? 'selected' : '' }}>{{ __('items.operation_types.rent') }}</option>
                                <option value="donate" {{ $item->operation_type->value == 'donate' ? 'selected' : '' }}>{{ __('items.operation_types.donate') }}</option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" rows="4">{{ old('description', $item->description) }}</textarea>
                        ...
                        <div class="mb-4" id="price-field">
                            ...
                            <x-text-input id="price" name="price" type="number" step="0.01" class="mt-1 block w-full" :value="old('price', $item->price)" />
                        ...
                        <div class="mb-4" id="deposit-field" style="display: none;">
                            ...
                            <x-text-input id="deposit_amount" name="deposit_amount" type="number" step="0.01" class="mt-1 block w-full" :value="old('deposit_amount', $item->deposit_amount)" />
                            <x-input-error :messages="$errors->get('deposit_amount')" class="mt-2" />
                        </div>

                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('items.show', $item) }}" class="mr-2">
