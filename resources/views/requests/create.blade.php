...
@section('title', 'إنشاء طلب جديد - ' . config('app.name'))
...
<div class="khezana-create-page">
    <div class="khezana-container">
        ...
        <div class="khezana-page-header">
            <h1 class="khezana-page-title">إنشاء طلب جديد</h1>
            <p class="khezana-page-subtitle">
                اكتب طلبك بالتفصيل وسيجدك الآخرون
            </p>
        </div>
            ...
            <form method="POST" action="{{ route('requests.store') }}" class="khezana-form">
                ...
                <div class="khezana-form-group">
                    ...
                    <select 
                        name="category_id" 
                        id="category_id" 
                        class="khezana-form-input khezana-form-select"
                        required
                        ...
                        @foreach($categories as $category)
                            @php
                                ...
                                $allAttrs->each(function($attr) {
                                    if ($attr instanceof \App\Models\Attribute && !$attr->relationLoaded('values')) {
                                        $attr->load('values');
                                    }
                                });
                                $categoryAttributes = $allAttrs->filter(function($attr) {
                            ...
                            @if($category->children->count() > 0)
                                @foreach($category->children as $child)
                                    @php
                                        ...
                                        $childAttributes = $childAllAttrs->filter(function($attr) {
                                            return $attr instanceof \App\Models\Attribute;
                                        })->map(function($attr) {
                                            return [
                                                'id' => $attr->id,
                ...
                <div class="khezana-form-group">
                    <label for="title" class="khezana-form-label">
                        {{ __('requests.fields.title') ?? 'عنوان الطلب' }}
                        <span class="khezana-required">*</span>
                    </label>
                    <input 
                        type="text" 
                ...
                <div class="khezana-form-group">
                    <label for="description" class="khezana-form-label">
                        {{ __('requests.fields.description') ?? 'الوصف' }}
                    </label>
                    <textarea 
                        name="description" 
                        id="description" 
                        class="khezana-form-input khezana-form-textarea"
                        rows="5"
                ...
                <div class="khezana-form-actions">
                    <button type="submit" class="khezana-btn khezana-btn-primary khezana-btn-large">
                        {{ __('requests.actions.submit_for_approval') ?? 'إرسال الطلب' }}
                    </button>
